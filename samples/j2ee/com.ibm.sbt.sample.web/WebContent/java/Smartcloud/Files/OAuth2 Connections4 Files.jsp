...
<h2>Click on button to get files</h2>
	...
	<form name="getProfileForm" method="post" action="" >
		...
		<button  name="UserBtn" type="button" onclick="setEmailValue('FrankAdams@renovations.com');">
		Click here to view Files from Frank
		</button>
	</form>
	<br>
	...
	<%
		if (request.getParameter("emailValue") != null) {
			...
			if (request.getParameter("emailValue") != null)
				parameters.put("startIndex", request.getParameter("0"));
			Object result = cs.get(filesUrl,parameters,ClientService.FORMAT_XML);
			
			Element root = ((Document)result).getDocumentElement();
		    NodeList titlenodes = ((Document)result).getElementsByTagName("title");
		    out.println("We found <b>"+(titlenodes.getLength()-2)+"</b> File uploaded by Frank <br>"); 
		    for(int index=2;index<titlenodes.getLength();index++){
		    	Node node = (Node)titlenodes.item(index);
		    	out.println("<h3>"+(index-1) +" : "+ DOMUtil.getTextValue(node)+"</h3>");
		    }
		}
	%>
