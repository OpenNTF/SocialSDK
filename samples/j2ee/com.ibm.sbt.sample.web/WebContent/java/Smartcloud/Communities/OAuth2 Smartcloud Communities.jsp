...
<body>
	...
	<%
		...
		try {
			...
			ep = EndpointFactory.getEndpoint("smartcloudOA2");
	...
	<%
		if (ep != null) {
			try {
				...
				Object result = svc.get(myCommunitiesURL, null,
						ClientService.FORMAT_XML); // Get my Communities, Format should be XML
				...
				out.println("We found <b>" + (titlenodes.getLength() - 1)
						+ "</b> communities for you<br><br>");
				for (int index = 1; index < titlenodes.getLength(); index++) {
					Node node = (Node) titlenodes.item(index);
					out.println(index + " : " + DOMUtil.getTextValue(node)
							+ "<br>");
				}
			} catch (Throwable e) {
				out.println("<pre>");
