		...
		case 'delete-selected':
			...
			<div class="wrap">
				***REMOVED***
				...
				?>
				***REMOVED*** if ( $have_non_network_plugins && is_network_admin() ) : ?>
				<div class="error"><p><strong>***REMOVED*** _e( 'Caution:' ); ?></strong> ***REMOVED*** echo _n( 'This plugin may be active on other sites in the network.', 'These plugins may be active on other sites in the network.', $plugins_to_delete ); ?></p></div>
				***REMOVED*** endif; ?>
				<p>***REMOVED*** echo _n( 'You are about to remove the following plugin:', 'You are about to remove the following plugins:', $plugins_to_delete ); ?></p>
					<ul class="ul-disc">
						***REMOVED***
				...
				?></p>
				<form method="post" action="***REMOVED*** echo esc_url($_SERVER['REQUEST_URI']); ?>" style="display:inline;">
					...
					***REMOVED***
					...
					?>
					***REMOVED*** wp_nonce_field('bulk-plugins') ?>
					***REMOVED*** submit_button( $data_to_delete ? __( 'Yes, Delete these files and data' ) : __( 'Yes, Delete these files' ), 'button', 'submit', false ); ?>
				</form>
				<form method="post" action="***REMOVED*** echo esc_url(wp_get_referer()); ?>" style="display:inline;">
					***REMOVED*** submit_button( __( 'No, Return me to the plugin list' ), 'button', 'submit', false ); ?>
				</form>

				<p><a href="#" onclick="jQuery('#files-list').toggle(); return false;">***REMOVED*** _e('Click to view entire list of files which will be deleted'); ?></a></p>
...
***REMOVED*** if ( isset($_GET['error']) ) :
	...
	?>
	<div id="message" class="updated"><p>***REMOVED*** echo $errmsg; ?></p>
	***REMOVED***
		if ( !isset( $_GET['main'] ) && !isset($_GET['charsout']) && wp_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $plugin) ) { ?>
	<iframe style="border:0" width="100%" height="70px" src="***REMOVED*** echo 'plugins.php?action=error_scrape&amp;plugin=' . esc_attr($plugin) . '&amp;_wpnonce=' . esc_attr($_GET['_error_nonce']); ?>"></iframe>
	***REMOVED***
		}
	?>
	</div>
***REMOVED*** elseif ( isset($_GET['deleted']) ) :
		$delete_result = get_transient('plugins_delete_result_'.$user_ID);
		delete_transient('plugins_delete_result'); //Delete it once we're done.

		if ( is_wp_error($delete_result) ) : ?>
		<div id="message" class="updated"><p>***REMOVED*** printf( __('Plugin could not be deleted due to an error: %s'), $delete_result->get_error_message() ); ?></p></div>
		***REMOVED*** else : ?>
		<div id="message" class="updated"><p>***REMOVED*** _e('The selected plugins have been <strong>deleted</strong>.'); ?></p></div>
		***REMOVED*** endif; ?>
***REMOVED*** elseif ( isset($_GET['activate']) ) : ?>
	<div id="message" class="updated"><p>***REMOVED*** _e('Plugin <strong>activated</strong>.') ?></p></div>
***REMOVED*** elseif (isset($_GET['activate-multi'])) : ?>
	<div id="message" class="updated"><p>***REMOVED*** _e('Selected plugins <strong>activated</strong>.'); ?></p></div>
***REMOVED*** elseif ( isset($_GET['deactivate']) ) : ?>
	<div id="message" class="updated"><p>***REMOVED*** _e('Plugin <strong>deactivated</strong>.') ?></p></div>
***REMOVED*** elseif (isset($_GET['deactivate-multi'])) : ?>
	<div id="message" class="updated"><p>***REMOVED*** _e('Selected plugins <strong>deactivated</strong>.'); ?></p></div>
***REMOVED*** elseif ( 'update-selected' == $action ) : ?>
	<div id="message" class="updated"><p>***REMOVED*** _e('No out of date plugins were selected.'); ?></p></div>
***REMOVED*** endif; ?>
...
if ( ( ! is_multisite() || is_network_admin() ) && current_user_can('install_plugins') ) { ?>
 <a href="***REMOVED*** echo self_admin_url( 'plugin-install.php' ); ?>" class="add-new-h2">***REMOVED*** echo esc_html_x('Add New', 'plugin'); ?></a>
***REMOVED*** }
if ( $s )
	printf( '<span class="subtitle">' . __('Search results for &#8220;%s&#8221;') . '</span>', esc_html( $s ) ); ?>
</h2>

***REMOVED*** do_action( 'pre_current_active_plugins', $plugins['all'] ) ?>

***REMOVED*** $wp_list_table->views(); ?>

<form method="get" action="">
***REMOVED*** $wp_list_table->search_box( __( 'Search Installed Plugins' ), 'plugin' ); ?>
</form>

<form method="post" action="">

<input type="hidden" name="plugin_status" value="***REMOVED*** echo esc_attr($status) ?>" />
<input type="hidden" name="paged" value="***REMOVED*** echo esc_attr($page) ?>" />
