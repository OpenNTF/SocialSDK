...
function network_step1( $errors = false ) {
	...
	?>
	<p>***REMOVED*** _e( 'Welcome to the Network installation process!' ); ?></p>
	<p>***REMOVED*** _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of WordPress sites. We will create configuration files in the next step.' ); ?></p>
	***REMOVED***
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) : ?>
		<h3>***REMOVED*** esc_html_e( 'Addresses of Sites in your Network' ); ?></h3>
		<p>***REMOVED*** _e( 'Please choose whether you would like sites in your WordPress network to use sub-domains or sub-directories. <strong>You cannot change this later.</strong>' ); ?></p>
		<p>***REMOVED*** _e( 'You will need a wildcard DNS record if you are going to use the virtual host (sub-domain) functionality.' ); ?></p>
		***REMOVED*** // @todo: Link to an MS readme? ?>
		<table class="form-table">
			<tr>
				<th><label><input type='radio' name='subdomain_install' value='1'***REMOVED*** checked( $subdomain_install ); ?> /> ***REMOVED*** _e( 'Sub-domains' ); ?></label></th>
				<td>***REMOVED*** printf( _x( 'like <code>site1.%1$s</code> and <code>site2.%1$s</code>', 'subdomain examples' ), $hostname ); ?></td>
			</tr>
			<tr>
				<th><label><input type='radio' name='subdomain_install' value='0'***REMOVED*** checked( ! $subdomain_install ); ?> /> ***REMOVED*** _e( 'Sub-directories' ); ?></label></th>
				<td>***REMOVED*** printf( _x( 'like <code>%1$s/site1</code> and <code>%1$s/site2</code>', 'subdirectory examples' ), $hostname ); ?></td>
			</tr>
		</table>
...
***REMOVED***
		...
		if ( $is_www ) :
		?>
		<h3>***REMOVED*** esc_html_e( 'Server Address' ); ?></h3>
		<p>***REMOVED*** printf( __( 'We recommend you change your siteurl to <code>%1$s</code> before enabling the network feature. It will still be possible to visit your site using the <code>www</code> prefix with an address like <code>%2$s</code> but any links will not have the <code>www</code> prefix.' ), substr( $hostname, 4 ), $hostname ); ?></p>
		<table class="form-table">
			<tr>
				<th scope='row'>***REMOVED*** esc_html_e( 'Server Address' ); ?></th>
				<td>
					***REMOVED*** printf( __( 'The internet address of your network will be <code>%s</code>.' ), $hostname ); ?>
				</td>
			</tr>
		</table>
		***REMOVED*** endif; ?>

		<h3>***REMOVED*** esc_html_e( 'Network Details' ); ?></h3>
		<table class="form-table">
		***REMOVED*** if ( 'localhost' == $hostname ) : ?>
			<tr>
				<th scope="row">***REMOVED*** esc_html_e( 'Sub-directory Install' ); ?></th>
				<td>***REMOVED***
		...
		***REMOVED*** elseif ( !allow_subdomain_install() ) : ?>
			<tr>
				<th scope="row">***REMOVED*** esc_html_e( 'Sub-directory Install' ); ?></th>
				<td>***REMOVED***
		...
		***REMOVED*** elseif ( !allow_subdirectory_install() ) : ?>
			<tr>
				<th scope="row">***REMOVED*** esc_html_e( 'Sub-domain Install' ); ?></th>
				<td>***REMOVED*** _e( 'Because your install is not new, the sites in your WordPress network must use sub-domains.' );
		...
		***REMOVED*** endif; ?>
			<tr>
				<th scope='row'>***REMOVED*** esc_html_e( 'Network Title' ); ?></th>
				<td>
					<input name='sitename' type='text' size='45' value='***REMOVED*** echo esc_attr( $site_name ); ?>' />
					<br />***REMOVED*** _e( 'What would you like to call your network?' ); ?>
				</td>
			</tr>
			<tr>
				<th scope='row'>***REMOVED*** esc_html_e( 'Admin E-mail Address' ); ?></th>
				<td>
					<input name='email' type='text' size='45' value='***REMOVED*** echo esc_attr( $admin_email ); ?>' />
					<br />***REMOVED*** _e( 'Your email address.' ); ?>
				</td>
			</tr>
		</table>
		***REMOVED*** submit_button( __( 'Install' ), 'primary', 'submit' ); ?>
	</form>
	***REMOVED***
}
...
?>
	<p>***REMOVED*** _e( 'The original configuration steps are shown here for reference.' ); ?></p>
***REMOVED***
		} else {
			$subdomain_install = (bool) $wpdb->get_var( "SELECT meta_value FROM $wpdb->sitemeta WHERE site_id = 1 AND meta_key = 'subdomain_install'" );
?>
	<div class="error"><p><strong>***REMOVED*** _e('Warning:'); ?></strong> ***REMOVED*** _e( 'An existing WordPress network was detected.' ); ?></p></div>
	<p>***REMOVED*** _e( 'Please complete the configuration steps. To create a new network, you will need to empty or remove the network database tables.' ); ?></p>
***REMOVED***
		}
	}
...
?>
		<h3>***REMOVED*** esc_html_e( 'Enabling the Network' ); ?></h3>
		<p>***REMOVED*** _e( 'Complete the following steps to enable the features for creating a network of sites.' ); ?></p>
		<div class="updated inline"><p>***REMOVED***
...
?>
		<ol>
			<li><p>***REMOVED*** printf( __( 'Add the following to your <code>wp-config.php</code> file in <code>%s</code> <strong>above</strong> the line reading <code>/* That&#8217;s all, stop editing! Happy blogging. */</code>:' ), $location_of_wp_config ); ?></p>
				<textarea class="code" readonly="readonly" cols="100" rows="6">
define('MULTISITE', true);
define('SUBDOMAIN_INSTALL', ***REMOVED*** echo $subdomain_install ? 'true' : 'false'; ?>);
define('DOMAIN_CURRENT_SITE', '***REMOVED*** echo $hostname; ?>');
define('PATH_CURRENT_SITE', '***REMOVED*** echo $base; ?>');
...
?>
	<p>***REMOVED***
		echo _n( 'This unique authentication key is also missing from your <code>wp-config.php</code> file.', 'These unique authentication keys are also missing from your <code>wp-config.php</code> file.', $num_keys_salts ); ?> ***REMOVED*** _e( 'To make your installation more secure, you should also add:' ) ?></p>
	<textarea class="code" readonly="readonly" cols="100" rows="***REMOVED*** echo $num_keys_salts; ?>">***REMOVED*** echo esc_textarea( $keys_salts_str ); ?></textarea>
***REMOVED***
	}
?>
</li>
...
<configuration>
	...
	?>
		<li><p>***REMOVED*** printf( __( 'Add the following to your <code>web.config</code> file in <code>%s</code>, replacing other WordPress rules:' ), $home_path ); ?></p>
		***REMOVED***
		if ( ! $subdomain_install && WP_CONTENT_DIR != ABSPATH . 'wp-content' )
			echo '<p><strong>' . __('Warning:') . ' ' . __( 'Subdirectory networks may not be fully compatible with custom wp-content directories.' ) . '</strong></p>';
		?>
		<textarea class="code" readonly="readonly" cols="100" rows="20">***REMOVED*** echo esc_textarea( $web_config_file ); ?>
		</textarea></li>
		</ol>

	***REMOVED*** else : // end iis7_supports_permalinks(). construct an htaccess file instead:
...
EOF;
		...
		?>
		<li><p>***REMOVED*** printf( __( 'Add the following to your <code>.htaccess</code> file in <code>%s</code>, replacing other WordPress rules:' ), $home_path ); ?></p>
		***REMOVED***
		if ( ! $subdomain_install && WP_CONTENT_DIR != ABSPATH . 'wp-content' )
			echo '<p><strong>' . __('Warning:') . ' ' . __( 'Subdirectory networks may not be fully compatible with custom wp-content directories.' ) . '</strong></p>';
		?>
		<textarea class="code" readonly="readonly" cols="100" rows="***REMOVED*** echo substr_count( $htaccess_file, "\n" ) + 1; ?>">
***REMOVED*** echo esc_textarea( $htaccess_file ); ?></textarea></li>
		</ol>

	***REMOVED*** endif; // end IIS/Apache code branches.

	if ( !is_multisite() ) { ?>
		<p>***REMOVED*** printf( __( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.') ); ?> <a href="***REMOVED*** echo esc_url( site_url( 'wp-login.php' ) ); ?>">***REMOVED*** _e( 'Log In' ); ?></a></p>
***REMOVED***
	}
}
