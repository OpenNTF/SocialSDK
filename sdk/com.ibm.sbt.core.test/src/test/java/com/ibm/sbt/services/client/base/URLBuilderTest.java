...
public class URLBuilderTest extends BaseUnitTest {
	...
	public void testGenerateURLPredefinedParameters() {
		...
		// URL pattern is
		// "communities/service/atom/{authType}/{communityEntity}/{communityType}"
		String url = CommunityUrls.COMMUNITY_MEMBERS.format(service,
				CommunityUrls.getCommunityUuid(communityUuid));
		assertEquals(
				"communities/service/atom/community/members?communityUuid=thisIsACommunityUuid",
				url);
	}
	...
	public void testGenerateURLVariableParameters() {
		...
		// URL pattern is
		// "wikis/{authType}/api/wiki/{wikiLabel}/page/{wikiPage}/entry"
		String wikiLabel = "myWikiLabel";
		String pageLabel = "myPageLabel";
		String url = WikiUrls.WIKI_PAGE.format(service,
				WikiUrls.getWikiLabel(wikiLabel),
				WikiUrls.getWikiPage(pageLabel));
		assertEquals("wikis/basic/api/wiki/" + wikiLabel + "/page/" + pageLabel
				+ "/entry", url);
	}
	...
	public void testGenerateURLUnsortedParameters() {
		...
		String wikiLabel = "myWikiLabel";
		String pageLabel = "myPageLabel";
		// This url has the correct parameters but in a different order. It
		// works regardless. The first parameter needs to be the Version, though
		String url = WikiUrls.WIKI_PAGE.format(service,
				WikiUrls.getWikiPage(pageLabel),
				WikiUrls.getWikiLabel(wikiLabel));
		assertEquals("wikis/basic/api/wiki/" + wikiLabel + "/page/" + pageLabel
	...
	public void testGenerateURLRigtNumberOfParametersButWrongParameter() {
		...
		String wikiLabel = "myWikiLabel";
		String pageLabel = "myPageLabel";
		// This url has the correct number of parameters but one of the
		// parameters is not correct
		thrown.expect(IllegalArgumentException.class);
		thrown.expectMessage("Missing parameter");
		String url = WikiUrls.WIKI_PAGE.format(service,
				WikiUrls.getWikiLabel(wikiLabel),
				FileUrlParts.fileId.get("dummy"));
		assertEquals("wikis/basic/api/wiki/" + wikiLabel + "/page/" + pageLabel
				+ "/entry", url);
	}
	...
	public void testGenerateURLVariableAndEmptyParameters() {
		...
		String url2 = WikiUrls.WIKI_PAGE.format(service,
				WikiUrls.getWikiLabel(""), WikiUrls.getWikiPage("myPageLabel"));
	...
	public void testGenerateURLMissingParameters() {
		...
		// String url =
		// FileUrls.SINGLE_COMMENT_USER_FILE.format(getApiVersion(),
		// getAuthTypeBasic(), AccessType.PUBLIC.get(),
		// FileUrlParts.userId.get(userId), FileUrlParts.fileId.get(fileId),
		// FileUrlParts.commentId.get(commentId));

		// This url misses the last parameter, commentId
		thrown.expect(IllegalArgumentException.class);
		thrown.expectMessage("Missing parameter");
		FileUrls.USERLIBRARY_DOCUMENT_FEED.format(service,
	...
	public void testGenerateURLExtraParameters() {
		...
		String url = FileUrls.USERLIBRARY_DOCUMENT_COMMENT_ENTRY.format(
				service, AccessType.PUBLIC.get(),
				FileUrlParts.userId.get(userId),
				FileUrlParts.fileId.get(fileId),
				FileUrlParts.commentId.get(commentId), ASUser.ME.get(),
				WikiUrls.getWikiLabel("value")); // The last 2 parameters are
													// not needed, they are
													// simply ignored
		String assertUrl = "files/basic/anonymous/api/userlibrary/" + userId
				+ "/document/" + fileId + "/comment/" + commentId + "/entry";
	...
	public void testGenerateURLVariableParameters2() {
		...
		// URL pattern is
		// "connections/opensocial/{authType}/rest/{service}/{user}/{group}/{application}"

		String user = "%40me";
		String group = "%40all";
		String app = "%40status";
		
		String urlAssert = "connections/opensocial/basic/rest/activitystreams/"
				+ user + "/" + group + "/" + app;
		String url = ActivityStreamUrls.AS_USER_GROUP_APP.format(service,
				ASUser.getByName("me"), ASGroup.getByName("all"),
				ASApplication.getByName("status"));
		assertEquals(urlAssert, url);

		String url2 = ActivityStreamUrls.AS_ME_ALL_STATUS.format(service);
		
		//URL Encodes with %40
		assertEquals(urlAssert.replace("%40","@"), url2);

		String community = "urn:lsid:lconn.ibm.com:communities.community:a1b2c3d4c5g6";
		String urlAssert2 = "connections/opensocial/basic/rest/activitystreams/"
				+ community + "/" + group;
		String url3 = ActivityStreamUrls.AS_COMMUNITY_ALL.format(service,
				ASUser.COMMUNITY.getWithValue("a1b2c3d4c5g6"));
		
		//URL Encodes with %40
		assertEquals(urlAssert2.replace("%40","@"), url3);
	}
	...
	public void testGenerateUrlWithUrlParameters() {
		...
		String url = ProfileUrls.CHECK_COLLEAGUE.format(service,
	...
	public void testGenerateUrlWithMissingUrlParameters() {
		...
		thrown.expect(IllegalArgumentException.class);
		thrown.expectMessage("Missing parameter");
		ProfileUrls.CHECK_COLLEAGUE.format(service,
				ProfileParams.sourceId.get(sourceId));
	}
	...
	public void testGenerateUrlWithUrlParametersNameAndValue() {
		...
		String url = ProfileUrls.CONNECTION.format(service,
				ProfileUrls.getConnectionId("12345678"));
		assertEquals(urlAssert, url);
	}
}
