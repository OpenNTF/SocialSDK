...
public class OAuth1Handler extends OAuthHandler {
	...
	public void getRequestTokenFromServer() throws Exception {
		...
		try {
			...
			url.append(Configuration.CONSUMER_KEY).append('=').append(URLEncoder.encode(oaProvider.getConsumerKey(),"UTF-8")).append('&');
			url.append(Configuration.SIGNATURE).append('=').append(URLEncoder.encode(buildSignature(oaProvider.getConsumerSecret(), ""),"UTF-8")).append('&');
			url.append(Configuration.SIGNATURE_METHOD).append('=').append(URLEncoder.encode(Configuration.PLAINTEXT_SIGNATURE,"UTF-8")).append('&');
			url.append(Configuration.TIMESTAMP).append('=').append(URLEncoder.encode(getTimestamp(),"UTF-8")).append('&');
			url.append(Configuration.NONCE).append('=').append(URLEncoder.encode(getNonce(),"UTF-8")).append('&');
			url.append(Configuration.VERSION).append('=').append(URLEncoder.encode(Configuration.OAUTH_VERSION1,"UTF-8")).append('&');
			url.append(Configuration.CALLBACK).append('=').append(URLEncoder.encode(oaProvider.getCallbackUrl(context),"UTF-8"));
		...
		if (responseCode != HttpStatus.SC_OK) {
			if (responseCode == HttpStatus.SC_NOT_IMPLEMENTED) {
				throw new Exception("getRequestToken failed with Response Code: Not implemented (501),<br>Msg: "+responseBody);
			} else if (responseCode == HttpStatus.SC_UNAUTHORIZED) {
				throw new Exception("getRequestToken failed with Response Code: Unauthorized (401),<br>Msg: "+responseBody);
			} else if (responseCode == HttpStatus.SC_BAD_REQUEST) {
				throw new Exception("getRequestToken failed with Response Code: Bad Request (400),<br>Msg: "+responseBody.toString());
			} else if (responseCode == HttpStatus.SC_INTERNAL_SERVER_ERROR) {
				throw new Exception("getRequestToken failed with Response Code: Internal Server error (500),<br>Msg: "+responseBody);
		    } else {
				throw new Exception("getRequestToken failed with Response Code (" + responseCode + "),<br>Msg: " + responseBody);
		    }
		} else {
			...
			String issuedOnDate = getTokenValue(responseBody, Configuration.OAUTH1_ISSUEDON);
			//Date issuedOn = new Date(Long.valueOf(issuedOnDate));
			setExpiresIn(getTokenValue(responseBody, Configuration.OAUTH1_EXPIRESIN));		
		}
		if (logger.isLoggable(Level.FINEST)) {
    		logger.exiting(sourceClass, "getRequestTokenFromServer", new Object[] { getRequestToken(), getRequestTokenSecret(), responseBody, responseCode });
        }
	}
	...
	public void getAccessTokenFromServer() throws Exception {
		...
		try {
			...
			url.append(Configuration.CONSUMER_KEY).append('=').append(URLEncoder.encode(oaProvider.getConsumerKey(),"UTF-8")).append('&');
			url.append(Configuration.OAUTH_TOKEN).append('=').append(URLEncoder.encode(requestToken,"UTF-8")).append('&');
			url.append(Configuration.OAUTH_VERIFIER).append('=').append(URLEncoder.encode(verifierCode,"UTF-8")).append('&');
			url.append(Configuration.SIGNATURE).append('=').append(URLEncoder.encode(buildSignature(oaProvider.getConsumerSecret(), requestTokenSecret),"UTF-8")).append('&');
			url.append(Configuration.SIGNATURE_METHOD).append('=').append(URLEncoder.encode(Configuration.PLAINTEXT_SIGNATURE,"UTF-8")).append('&');
			url.append(Configuration.TIMESTAMP).append('=').append(URLEncoder.encode(getTimestamp(),"UTF-8")).append('&');
			url.append(Configuration.NONCE).append('=').append(URLEncoder.encode(getNonce(),"UTF-8")).append('&');
			url.append(Configuration.VERSION).append('=').append(URLEncoder.encode(Configuration.OAUTH_VERSION1,"UTF-8"));
			method = new HttpGet(url.toString());
		...
		if (responseCode != HttpStatus.SC_OK) {
			if (responseCode == HttpStatus.SC_NOT_IMPLEMENTED) {
				...
				throw new Exception("getRequestToken failed with Response Code: Bad Request (400),<br>Msg: "+responseBody);
			} else if (responseCode == HttpStatus.SC_INTERNAL_SERVER_ERROR) {
				throw new Exception("getRequestToken failed with Response Code: Internal Server error (500),<br>Msg: "+responseBody);
		    } else {
				throw new Exception("getRequestToken failed with Response Code (" + responseCode + "),<br>Msg: " + responseBody);
		...
		if (logger.isLoggable(Level.FINEST)) {
    		logger.exiting(sourceClass, "getAccessTokenFromServer", new Object[] { getAccessToken(), getAccessTokenSecret(), responseBody, responseCode });
        }
	}
	...
	public String createAuthorizationHeader() {
		...
		authHdr.append(Configuration.SIGNATURE).append("='").append(percentEncode(buildSignature(oaprovider.getConsumerSecret(), accessTokenSecret))).append("',");
