	...
	try {
		...
		if (!subscriberList.isEmpty()) {
			...
			try{
    			...
    			out.println("Error assigning roles caused by: <pre>" + JsonGenerator.toJson(JsonJavaFactory.instanceEx, jsonObject, false) + "</pre>");
			}
			...
			StateChangeListener stateChangeListener = new StateChangeListener() {
				...
				public void stateChanged(JsonEntity jsonEntity) {
					try {
						...
						fout.println("Assigned role(s) as requested <br/>");
			...
			if (!authorizationService.waitRoleSetState(loginName,
				...
				out.println("Timeout waiting for roles to be assigned");
			}
			
		}
	} catch (Exception e) {
		e.printStackTrace();
		out.println("Error assigning roles caused by: "+e.getMessage());    		
	}
