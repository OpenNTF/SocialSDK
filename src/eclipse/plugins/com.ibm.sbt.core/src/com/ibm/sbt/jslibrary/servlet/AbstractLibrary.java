...
/**
 * Handles initialising the Social Business Toolkit library with the appropriate
 * bridge to the underlying JavaScript library i.e. dojo or jquery or ...
 * 
...
abstract public class AbstractLibrary {
    ...
    static private final String[][] REGISTER_EXT_MODULES = { 
        { MODULE_SBTX, PATH_SBTX } };
    ...
    /**
     * Return true if the associated <code>BaseLibrary</code> can handle this
     * request.
     * 
     * @param request
     * 
    ...
    /**
     * Generate the JavaScript to be returned for the specified
     * <code>LibraryRequest</code>
     * 
     * @param request
     *            LibraryRequest for which the script is being created
     * @param endpoints
     *            Map of JSonObjects representing the endpoints to be created in
     *            the JavaScript
     * @param properties
     *            JSonObject representing the properties to be included in the
     *            script
     * 
     * @throws LibraryException
    ...
    protected String generateJavaScript(LibraryRequest request, Map<String, JsonObject> endpoints, JsonObject properties) throws LibraryException {
        ...
        if (enableDefineCheck(request.getJsVersion())) {
            ...
            generateRegisterModules(sb, "    ", request, registerModules, false);
            if (StringUtil.isNotEmpty(request.getToolkitExtUrl())) {
                String[][] registerExtModules = getRegisterExtModules();
                generateRegisterModules(sb, "    ", request, registerExtModules, true);
            }
            ...
            generateRegisterModules(sb, "    ", request, registerModulesAmd, false);
            if (StringUtil.isNotEmpty(request.getToolkitExtUrl())) {
                String[][] registerExtModules = getRegisterExtModules();
                generateRegisterModules(sb, "    ", request, registerExtModules, true);
            }
     ...
     */
    protected void generateRegisterModules(StringBuilder sb, String padding, LibraryRequest request, String[][] registerModules, boolean isExtension) {
    ...
    /**
     * Convert the endpoint json to a new object which can be used to create
     * endpoint parameters
     * 
     * @param jsonObject
     * @return
    ...
    /**
     * Utility for changing a string so that its compatbile in javascript and
     * does not prevent backslashes
     * 
     * @param StringBuilder
     * @param string
