...
<div class="alignleft">
<h3>***REMOVED*** echo $theme->display('Name'); if ( $description ) echo ': ' . $description; ?></h3>
</div>
<div class="alignright">
	<form action="theme-editor.php" method="post">
		<strong><label for="theme">***REMOVED*** _e('Select theme to edit:'); ?> </label></strong>
...
?>
	<h3>***REMOVED*** _e('Templates'); ?></h3>
	***REMOVED*** if ( $theme->parent() ) : ?>
	<p class="howto">***REMOVED*** printf( __( 'This child theme inherits templates from a parent theme, %s.' ), '<a href="' . self_admin_url('theme-editor.php?theme=' . urlencode( $theme->get_template() ) ) . '">' . $theme->parent()->display('Name') . '</a>' ); ?></p>
	***REMOVED*** endif; ?>
	<ul>
***REMOVED***
	endif;
...
?>
		<li><a href="theme-editor.php?file=***REMOVED*** echo urlencode( $filename ) ?>&amp;theme=***REMOVED*** echo urlencode( $stylesheet ) ?>">***REMOVED*** echo $file_description; ?></a></li>
***REMOVED***
	endforeach;
?>
</ul>
...
***REMOVED*** if ( $error ) :
	...
	***REMOVED*** wp_nonce_field( 'edit-theme_' . $file . $stylesheet ); ?>
		<div><textarea cols="70" rows="30" name="newcontent" id="newcontent" aria-describedby="newcontent-description">***REMOVED*** echo $content; ?></textarea>
		<input type="hidden" name="action" value="update" />
		<input type="hidden" name="file" value="***REMOVED*** echo esc_attr( $relative_file ); ?>" />
		<input type="hidden" name="theme" value="***REMOVED*** echo esc_attr( $theme->get_stylesheet() ); ?>" />
		<input type="hidden" name="scrollto" id="scrollto" value="***REMOVED*** echo $scrollto; ?>" />
		</div>
	***REMOVED*** if ( ! empty( $functions ) ) : ?>
		<div id="documentation" class="hide-if-no-js">
		<label for="docs-list">***REMOVED*** _e('Documentation:') ?></label>
		***REMOVED*** echo $docs_select; ?>
		<input type="button" class="button" value=" ***REMOVED*** esc_attr_e( 'Look Up' ); ?> " onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'http://api.wordpress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=***REMOVED*** echo urlencode( get_locale() ) ?>&amp;version=***REMOVED*** echo urlencode( $wp_version ) ?>&amp;redirect=true'); }" />
		</div>
	***REMOVED*** endif; ?>

		<div>
		***REMOVED*** if ( is_child_theme() && $theme->get_stylesheet() == get_template() ) : ?>
			<p>***REMOVED*** if ( is_writeable( $file ) ) { ?><strong>***REMOVED*** _e( 'Caution:' ); ?></strong>***REMOVED*** } ?>
			***REMOVED*** _e( 'This is a file in your current parent theme.' ); ?></p>
		***REMOVED*** endif; ?>
***REMOVED***
	if ( is_writeable( $file ) ) :
