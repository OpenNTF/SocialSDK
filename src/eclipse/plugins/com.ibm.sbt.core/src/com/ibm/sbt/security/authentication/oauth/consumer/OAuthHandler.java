...
public abstract class OAuthHandler {
	...
	/**
	 * OAuth data is passed to other applications (e.g. API Explorer) to convey OAuth info
	 * that has already been collected from the user, so that the other application 
	 * does not have to collect it again ("poor man's" SSO).
	 */
...
/*	public ApiRequest getApiRequest() {
	...
	public static String getTokenValue(String input, String tokenName) {
		...
		String splitterkey = input.contains("&")?"&":","; // Connections and Smartcloud have different response types
	...
	public static String percentEncode(String str) {
		...
		try {
			encodedStr = URLEncoder.encode(str, Configuration.ENCODING).replace("+", "%20").replace("*", "%2A").replace("%7E", "~");
