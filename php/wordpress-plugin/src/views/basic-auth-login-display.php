		...
		require([ "sbt/dom", "sbt/config" ],
			...
			function(dom, config) {
				...
				endpoint.isAuthenticationValid({	
					...
					"actionUrl": "<?php echo plugins_url('sbtk-wp'); ?>/index.php?classpath=services&class=Proxy&method=route&isAuthenticated=true"}).then(
						function(response) {
							if (response.result) {
...
// 				config.Properties["loginPage"] = "/js/sdk/sbt/authenticator/templates/login.html";
			...
			function displayAccessGranted(dom, config) {
        		...
        		for (var i = 0; i < logoutBtn.length; i++) {
        			...
        			logoutBtn[i].onclick = function(evt) {	
        				var endpoint = config.findEndpoint("connections");
        				endpoint.logout({"actionUrl": "<?php echo plugins_url('sbtk-wp'); ?>/index.php?classpath=services&class=Proxy&method=route&basicAuthLogout=true"}).then(
        						function(response) {
        							displayGrantAccess(dom, config);
        							var logoutBtn = document.getElementsByName("logoutBtn");
