...
<div class="wrap">
<h2>***REMOVED*** _e('Caught Spam') ?></h2>
***REMOVED***
$count = get_option( 'akismet_spam_count' );
if ( $count ) {
?>
<p>***REMOVED*** printf(__('Akismet has caught <strong>%1$s spam</strong> for you since you first installed it.'), number_format_i18n($count) ); ?></p>
***REMOVED***
}
...
?>
***REMOVED*** if ( !isset( $_POST['s'] ) ) { ?>
<form method="post" action="***REMOVED*** echo attribute_escape( add_query_arg( 'noheader', 'true' ) ); ?>">
***REMOVED*** akismet_nonce_field($akismet_nonce) ?>
<input type="hidden" name="action" value="delete" />
***REMOVED*** printf(__('There are currently %1$s comments identified as spam.'), $spam_count); ?>&nbsp; &nbsp; <input type="submit" class="button delete" name="Submit" value="***REMOVED*** _e('Delete all'); ?>" />
<input type="hidden" name="display_time" value="***REMOVED*** echo current_time('mysql', 1); ?>" />
</form>
***REMOVED*** } ?>
</div>
<div class="wrap">
***REMOVED*** if ( isset( $_POST['s'] ) ) { ?>
<h2>***REMOVED*** _e('Search'); ?></h2>
***REMOVED*** } else { ?>
***REMOVED*** echo '<p>'.__('These are the latest comments identified as spam by Akismet. If you see any mistakes, simply mark the comment as "not spam" and Akismet will learn from the submission. If you wish to recover a comment from spam, simply select the comment, and click Not Spam. After 15 days we clean out the junk for you.').'</p>'; ?>
***REMOVED*** } ?>
***REMOVED***
if ( isset( $_POST['s'] ) ) {
...
<li ***REMOVED*** if ( !isset( $_GET['ctype'] ) ) echo ' class="active"'; ?>><a href="edit-comments.php?page=akismet-admin">***REMOVED*** _e('All'); ?></a></li>
***REMOVED***
...
if ($comments) {
?>
<form method="post" action="***REMOVED*** echo attribute_escape("$link?page=akismet-admin"); ?>" id="akismetsearch">
<p>  <input type="text" name="s" value="***REMOVED*** if (isset($_POST['s'])) echo attribute_escape($_POST['s']); ?>" size="17" />
  <input type="submit" class="button" name="submit" value="***REMOVED*** echo attribute_escape(__('Search Spam &raquo;')) ?>"  />  </p>
</form>
***REMOVED*** if ( $total > 50 ) {
...
***REMOVED*** } ?>
<form style="clear: both;" method="post" action="***REMOVED*** echo attribute_escape( add_query_arg( 'noheader', 'true' ) ); ?>">
***REMOVED*** akismet_nonce_field($akismet_nonce) ?>
...
<p><strong>***REMOVED*** comment_author() ?></strong> ***REMOVED*** if ($comment->comment_author_email) { ?>| ***REMOVED*** comment_author_email_link() ?> ***REMOVED*** } if ($comment->comment_author_url && 'http://' != $comment->comment_author_url) { ?> | ***REMOVED*** comment_author_url_link() ?> ***REMOVED*** } ?>| ***REMOVED*** _e('IP:') ?> <a href="http://ws.arin.net/cgi-bin/whois.pl?queryinput=***REMOVED*** comment_author_IP() ?>">***REMOVED*** comment_author_IP() ?></a></p>

***REMOVED*** comment_text() ?>

<p><label for="spam-***REMOVED*** echo $comment->comment_ID; ?>">
<input type="checkbox" id="spam-***REMOVED*** echo $comment->comment_ID; ?>" name="not_spam[]" value="***REMOVED*** echo $comment->comment_ID; ?>" />
***REMOVED*** _e('Not Spam') ?></label> &#8212; ***REMOVED*** comment_date('M j, g:i A');  ?> &#8212; [
***REMOVED***
$post = get_post($comment->comment_post_ID);
$post_title = wp_specialchars( $post->post_title, 'double' );
$post_title = ('' == $post_title) ? "# $comment->comment_post_ID" : $post_title;
?>
 <a href="***REMOVED*** echo get_permalink($comment->comment_post_ID); ?>" title="***REMOVED*** echo $post_title; ?>">***REMOVED*** _e('View Post') ?></a> ] </p>
...
?>
<p class="submit">
<input type="submit" name="submit" value="***REMOVED*** echo attribute_escape(__('De-spam marked comments &raquo;')); ?>" />
</p>
<p>***REMOVED*** _e('Comments you de-spam will be submitted to Akismet as mistakes so it can learn and get better.'); ?></p>
</form>
***REMOVED***
} else {
?>
<p>***REMOVED*** _e('No results found.'); ?></p>
***REMOVED*** } ?>
...
***REMOVED*** printf(__('There are currently %1$s comments identified as spam.'), $spam_count); ?>&nbsp; &nbsp; <input type="submit" name="Submit" class="button" value="***REMOVED*** echo attribute_escape(__('Delete all')); ?>" />
<input type="hidden" name="display_time" value="***REMOVED*** echo current_time('mysql', 1); ?>" /></p>
</form>
***REMOVED*** } ?>
</div>
***REMOVED***
