...
<html>
<head>
<title>Unsuspend Subscription</title>
...
<body>
	...
	try {
		...
		JsonJavaObject subscriptionObject = rootObject.getAsObject("Subscription");
 		...
 		if(!subscriptionObject.get("SubscriptionState").equals("SUSPENDED")){
		...
		StateChangeListener stateChangeListener = new StateChangeListener(){
			...
			public void stateChanged(JsonEntity jsonEntity){
				try{
					subscriptionManagement.unsuspendSubscription(subscriptId);
					fout.println("Unsuspended Subscription Id " + subscriptId + "<br/>");
				}catch(Exception e){
					try{
		...
		if(!subscriptionManagement.waitSubscriptionState(subscriptionId,"SUSPENDED",5,1000,stateChangeListener)){
			out.println("Timeout waiting for subscription to be suspended");
		}
	}	
	catch (BssException be) {
		out.println("<pre>");
		out.println("Error unsuspending subscription caused by: "+ be.getResponseJson());
		out.println("</pre>");	
	}
	catch (Throwable e) {
		e.printStackTrace();
		out.println("Error unsuspending subscription caused by: "+e.getMessage());    		
	}
	%>
	</div>
</body>
