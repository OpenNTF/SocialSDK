...
/* <![CDATA[ */
(function($){
	var default_color = '#***REMOVED*** echo get_theme_support( 'custom-header', 'default-text-color' ); ?>',
		header_text_fields;
	...
	$(document).ready(function() {
		...
		***REMOVED*** if ( ! display_header_text() ) : ?>
		toggle_text();
		***REMOVED*** endif; ?>
	});
})(jQuery);
...
/* <![CDATA[ */
	...
	jQuery(document).ready(function() {
		var xinit = ***REMOVED*** echo absint( get_theme_support( 'custom-header', 'width' ) ); ?>;
		var yinit = ***REMOVED*** echo absint( get_theme_support( 'custom-header', 'height' ) ); ?>;
		...
		jQuery('img#upload').imgAreaSelect({
			...
			***REMOVED***
			...
			?>
			maxHeight: ***REMOVED*** echo get_theme_support( 'custom-header', 'height' ); ?>,
			***REMOVED***
			}
			if ( ! current_theme_supports( 'custom-header', 'flex-width' ) ) {
			?>
			maxWidth: ***REMOVED*** echo get_theme_support( 'custom-header', 'width' ); ?>,
			***REMOVED***
			}
			?>
			onInit: function () {
...
<div class="wrap">
***REMOVED*** screen_icon(); ?>
<h2>***REMOVED*** _e('Custom Header'); ?></h2>

***REMOVED*** if ( ! empty( $this->updated ) ) { ?>
<div id="message" class="updated">
<p>***REMOVED*** printf( __( 'Header updated. <a href="%s">Visit your site</a> to see how it looks.' ), home_url( '/' ) ); ?></p>
</div>
***REMOVED*** } ?>

<h3>***REMOVED*** _e( 'Header Image' ); ?></h3>

<table class="form-table">
<tbody>

***REMOVED*** if ( get_custom_header() || display_header_text() ) : ?>
<tr valign="top">
<th scope="row">***REMOVED*** _e( 'Preview' ); ?></th>
<td>
	***REMOVED*** if ( $this->admin_image_div_callback ) {
	...
	?>
	<div id="headimg" style="***REMOVED*** echo $header_image_style; ?>">
		***REMOVED***
		...
		?>
		<h1><a id="name" class="displaying-header-text" ***REMOVED*** echo $style; ?> onclick="return false;" href="***REMOVED*** bloginfo('url'); ?>">***REMOVED*** bloginfo( 'name' ); ?></a></h1>
		<div id="desc" class="displaying-header-text" ***REMOVED*** echo $style; ?>>***REMOVED*** bloginfo( 'description' ); ?></div>
	</div>
	***REMOVED*** } ?>
</td>
</tr>
...
***REMOVED*** if ( current_theme_supports( 'custom-header', 'uploads' ) ) : ?>
<tr valign="top">
<th scope="row">***REMOVED*** _e( 'Select Image' ); ?></th>
<td>
	<p>***REMOVED*** _e( 'You can select an image to be shown at the top of your site by uploading from your computer or choosing from your media library. After selecting an image you will be able to crop it.' ); ?><br />
	***REMOVED***
	if ( ! current_theme_supports( 'custom-header', 'flex-height' ) && ! current_theme_supports( 'custom-header', 'flex-width' ) ) {
	...
	?></p>
	<form enctype="multipart/form-data" id="upload-form" class="wp-upload-form" method="post" action="***REMOVED*** echo esc_url( add_query_arg( 'step', 2 ) ) ?>">
	<p>
		<label for="upload">***REMOVED*** _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		***REMOVED*** wp_nonce_field( 'custom-header-upload', '_wpnonce-custom-header-upload' ); ?>
		***REMOVED*** submit_button( __( 'Upload' ), 'button', 'submit', false ); ?>
	</p>
	***REMOVED***
	...
	?>
	<p>
		<label for="choose-from-library-link">***REMOVED*** _e( 'Or choose an image from your media library:' ); ?></label><br />
		<a id="choose-from-library-link" class="button"
			data-update-link="***REMOVED*** echo esc_attr( $modal_update_href ); ?>"
			data-choose="***REMOVED*** esc_attr_e( 'Choose a Custom Header' ); ?>"
			data-update="***REMOVED*** esc_attr_e( 'Set as header' ); ?>">***REMOVED*** _e( 'Choose Image' ); ?></a>
	</p>
	</form>
</td>
</tr>
...
<form method="post" action="***REMOVED*** echo esc_url( add_query_arg( 'step', 1 ) ) ?>">
<table class="form-table">
<tbody>
	***REMOVED*** if ( get_uploaded_header_images() ) : ?>
<tr valign="top">
<th scope="row">***REMOVED*** _e( 'Uploaded Images' ); ?></th>
<td>
	<p>***REMOVED*** _e( 'You can choose one of your previously uploaded headers, or show a random one.' ) ?></p>
	***REMOVED***
...
<tr valign="top">
<th scope="row">***REMOVED*** _e( 'Default Images' ); ?></th>
<td>
***REMOVED*** if ( current_theme_supports( 'custom-header', 'uploads' ) ) : ?>
	<p>***REMOVED*** _e( 'If you don&lsquo;t want to upload your own image, you can use one of these cool headers, or show a random one.' ) ?></p>
***REMOVED*** else: ?>
	<p>***REMOVED*** _e( 'You can use one of these cool headers or show a random one on each page.' ) ?></p>
***REMOVED*** endif; ?>
	***REMOVED***
	...
	***REMOVED*** endif;
	if ( get_header_image() ) : ?>
<tr valign="top">
<th scope="row">***REMOVED*** _e( 'Remove Image' ); ?></th>
<td>
	<p>***REMOVED*** _e( 'This will remove the header image. You will not be able to restore any customizations.' ) ?></p>
	***REMOVED*** submit_button( __( 'Remove Header Image' ), 'button', 'removeheader', false ); ?>
</td>
</tr>
	***REMOVED*** endif;
...
<tr valign="top">
<th scope="row">***REMOVED*** _e( 'Reset Image' ); ?></th>
<td>
	<p>***REMOVED*** _e( 'This will restore the original header image. You will not be able to restore any customizations.' ) ?></p>
	***REMOVED*** submit_button( __( 'Restore Original Header Image' ), 'button', 'resetheader', false ); ?>
</td>
</tr>
	***REMOVED*** endif; ?>
</tbody>
...
***REMOVED*** if ( current_theme_supports( 'custom-header', 'header-text' ) ) : ?>

<h3>***REMOVED*** _e( 'Header Text' ); ?></h3>

<table class="form-table">
<tbody>
<tr valign="top">
<th scope="row">***REMOVED*** _e( 'Header Text' ); ?></th>
<td>
	<p>
	<label><input type="checkbox" name="display-header-text" id="display-header-text"***REMOVED*** checked( display_header_text() ); ?> /> ***REMOVED*** _e( 'Show header text with your image.' ); ?></label>
	</p>
</td>
</tr>

<tr valign="top" class="displaying-header-text">
<th scope="row">***REMOVED*** _e( 'Text Color' ); ?></th>
<td>
	<p>
***REMOVED***
...
***REMOVED*** submit_button( null, 'primary', 'save-header-options' ); ?>
</form>
</div>
...
<div class="wrap">
***REMOVED*** screen_icon(); ?>
<h2>***REMOVED*** _e( 'Crop Header Image' ); ?></h2>

<form method="post" action="***REMOVED*** echo esc_url(add_query_arg('step', 3)); ?>">
	<p class="hide-if-no-js">***REMOVED*** _e('Choose the part of the image you want to use as your header.'); ?></p>
	<p class="hide-if-js"><strong>***REMOVED*** _e( 'You need Javascript to choose a part of the image.'); ?></strong></p>

	<div id="crop_image" style="position: relative">
		<img src="***REMOVED*** echo esc_url( $url ); ?>" id="upload" width="***REMOVED*** echo $width; ?>" height="***REMOVED*** echo $height; ?>" />
	</div>

	<input type="hidden" name="x1" id="x1" value="0"/>
	<input type="hidden" name="y1" id="y1" value="0"/>
	<input type="hidden" name="width" id="width" value="***REMOVED*** echo esc_attr( $width ); ?>"/>
	<input type="hidden" name="height" id="height" value="***REMOVED*** echo esc_attr( $height ); ?>"/>
	<input type="hidden" name="attachment_id" id="attachment_id" value="***REMOVED*** echo esc_attr( $attachment_id ); ?>" />
	<input type="hidden" name="oitar" id="oitar" value="***REMOVED*** echo esc_attr( $oitar ); ?>" />
	***REMOVED*** if ( empty( $_POST ) && isset( $_GET['file'] ) ) { ?>
	<input type="hidden" name="create-new-attachment" value="true" />
	***REMOVED*** } ?>
	***REMOVED*** wp_nonce_field( 'custom-header-crop-image' ) ?>

	<p class="submit">
	***REMOVED*** submit_button( __( 'Crop and Publish' ), 'primary', 'submit', false ); ?>
	***REMOVED***
