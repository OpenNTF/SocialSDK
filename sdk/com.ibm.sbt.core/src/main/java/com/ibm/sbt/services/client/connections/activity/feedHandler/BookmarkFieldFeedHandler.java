...
/**
 * Feed Handler for Bookmark Fields
 * @author Vimal Dhupar
 *
 */
public class BookmarkFieldFeedHandler extends FieldFeedHandler implements IFeedHandler {
	 ...
	 * @param service
	 */
	public BookmarkFieldFeedHandler(ActivityService service){
		super(service);
	}
	...
	public Field createEntityFromData(Object data) {
		...
		XPathExpression expr = (data instanceof Document) ? (XPathExpression)ActivityXPath.field.getPath() : null;
		XmlDataHandler handler = new XmlDataHandler(node, ConnectionsConstants.nameSpaceCtx, expr);
		Field field = new BookmarkField(service, handler);
		return field;
	}
}