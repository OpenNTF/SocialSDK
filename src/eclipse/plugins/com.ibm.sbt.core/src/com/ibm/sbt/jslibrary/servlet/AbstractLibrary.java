...
/**
 * Handles initialising the Social Business Toolkit library with the appropriate bridge to
 * the underlying JavaScript library i.e. dojo or jquery or ...
 * 
 * This class need to be thread safe, only a single instance will be created.
 * 
 * @author mwallace
 */
abstract public class AbstractLibrary {
	...
	protected Map<String,JsonObject> populateEndpoints(LibraryRequest request) throws LibraryException {
		...
		if (endpoints != null) {
			for (com.ibm.sbt.jslibrary.SBTEnvironment.Endpoint endpoint : endpoints) {
				...
				Endpoint theEndpoint = EndpointFactory.getEndpointUnchecked(endpointName); // TODO Fix typo's
	 ...
	 */
	protected JsonObject createJsonForEndpoint(LibraryRequest request, Endpoint endpoint, String endpointAlias, String endpointName) throws LibraryException {
		if (logger.isLoggable(Level.FINEST)) {
			logger.entering(sourceClass, "createJsonForEndpoint", new Object[] {request, endpoint, endpointAlias, endpointName});
		}
		...
		if (isValid(request, endpoint)) {
			...
			jsonEndpoint.putJsonProperty(PROP_TRANSPORT, new JsonReference("new ErrorTransport('"+endpointName+"','"+message+"')"));
		}
	 ...
	 * 
	 * @param request LibraryRequest for which the script is being created
	 * @param endpoints Map of JSonObjects representing the endpoints to be created in the JavaScript
	 * @param properties JSonObject representing the properties to be included in the script
	 * 
	 * @throws LibraryException if an error occurs while generating the JavaScript
	 */
	protected String generateJavaScript(LibraryRequest request, Map<String, JsonObject> endpoints, JsonObject properties) throws LibraryException {
		if (logger.isLoggable(Level.FINEST)) {
			logger.entering(sourceClass, "generateJavaScript", new Object[] { request, endpoints, properties });
		}
		...
		if (enableDefineCheck(request.getJsVersion())) {
			sb.append("  if(typeof define=='undefined'){\n");

			// register the module paths and required modules
			String[][] registerModules = getRegisterModules();
			generateRegisterModules(sb, "    ", request, registerModules);
			String[] requireModules = getRequireModules();
			generateRequireModules(sb, "    ", requireModules);

			sb.append("  } else {\n");

			// register the module paths and required modules
			String[][] registerModulesAmd = getRegisterModulesAmd();
			generateRegisterModules(sb, "    ", request, registerModulesAmd);
			String[] requireModulesAmd = getRequireModulesAmd();
			generateRequireModules(sb, "    ", requireModulesAmd);
		...
		sb.append("  define('").append(getDefineModule()).append("',[").append(dmList).append("],function(").append(dmNames).append("){\n");

		// Create the sbt object
		sb.append("    window.sbt = {};\n");

		// define the properties
		Iterator<String> jsonProps = properties.getJsonProperties();
		if (jsonProps.hasNext()) {
			sb.append("    sbt.Properties=").append(toJson(properties)).append(";\n");
		}

		// define the endpoints
		if (!endpoints.isEmpty()) {
			sb.append("    sbt.Endpoints={\n");
			...
			while (entrySet.hasNext()) {
				...
				sb.append("      '").append(toJavaScriptString(name)).append("':new Endpoint(").append(toJson(endpointParams)).append(")");
				if (entrySet.hasNext()) {
					sb.append(",\n");
				}
				else {
	 ...
	 */
	protected void generateRequireModules(StringBuilder sb, String padding, String[] requireModules) {
		for (String requireModule : requireModules) {
			sb.append(padding).append(generateRequire(requireModule)).append(";\n");
		}
	}
	 ...
	 */
	protected void generateRegisterModules(StringBuilder sb, String padding, LibraryRequest request, String[][] registerModules) {
		for (String[] registerModule : registerModules) {
			String moduleUrl = getModuleUrl(request, registerModule[1]);
			sb.append(padding).append(generateRegisterModulePath(registerModule[0], moduleUrl)).append(";\n");
		}
	}
		...
		for (JsonObject jsonObject : jsonObjects) {
			if (jsonObject.getJsonProperty(PROP_INVALID) != null) {
				...
				if (jsonObject.getJsonProperty(PROP_AUTHENTICATOR) != null) {
					String authenticatorModule = (String)jsonObject.getJsonProperty(PROP_MODULE_AUTHENTICATOR);
	 ...
	 * @return
	 */
	protected String getModuleUrl(LibraryRequest request, String modulePath) {
		if (logger.isLoggable(Level.FINEST)) {
			logger.entering(sourceClass, "getModuleUrl", new Object[] { request, modulePath });
		}

		String toolkitJsUrl = request.getToolkitJsUrl();
	 ...
	 * @throws LibraryException
	 */
	protected JsonReference createAuthenticatorRef(LibraryRequest request, Endpoint endpoint, String logicalName) throws LibraryException {
		JSReference authenticator = endpoint.getAuthenticator(logicalName);
		if (authenticator != null) {
			try {
				String paramValues = JsonGenerator.toJson(JsonJavaFactory.instanceEx, authenticator.getProperties());
	...
	protected boolean useProxy(Endpoint endpoint) {
		if (endpoint instanceof GadgetEndpoint ||
				endpoint instanceof GadgetOAuthEndpoint) {
			return false;
		}
		return true;
	}
	 ...
	 * @throws LibraryException
	 */
	protected JsonReference createProxyRef(LibraryRequest request, Endpoint endpoint, String logicalName) throws LibraryException {
	 ...
	 * @param StringBuilder
	 * @param string converted to comply with javascript
	 */
	 ...
	 * @param buildingString String being constructed
	 * @param string converted to comply with javascript
