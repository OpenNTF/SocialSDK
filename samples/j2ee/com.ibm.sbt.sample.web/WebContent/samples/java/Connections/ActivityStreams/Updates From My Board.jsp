...
<body>
	<h4>Updates from My Board</h4>
	<div id="content">
	<%
		try {
			ActivityStreamService _service = new ActivityStreamService();
			List<ActivityStreamEntry> _entries = (List) _service.getUpdatesFromMyBoard();
			...
			for (Iterator iterator = _entries.iterator(); iterator.hasNext();) {
				...
				out.println(entry.getActor().getName() + " : " + entry.getSummary());
				out.println("<br>");
				...
				if(entry.getAttachment()!=null && entry.getAttachment().getUrl()!=null){
					out.println("Attachment : " + entry.getAttachment().getDisplayName() + "<br>");
				}
