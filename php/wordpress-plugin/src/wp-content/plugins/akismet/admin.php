...
<div class="wrap">
	***REMOVED*** if ( !$api_key ) : ?>
	<h2 class="ak-header">***REMOVED*** _e('Akismet'); ?></h2>
	***REMOVED*** else: ?>
	<h2 class="ak-header">***REMOVED*** printf( __( 'Akismet <a href="%s" class="add-new-h2">Stats</a>' ), $stats_link ); ?></h2>
	***REMOVED*** endif; ?>
	<div class="no-key ***REMOVED*** echo $show_key_form ? 'hidden' : '';?>">
		<p>***REMOVED*** _e('Akismet eliminates the comment and trackback spam you get on your site. To use Akismet you may need to sign up for an API key. Click the button below to get started.'); ?></p>
		<form name="akismet_activate" action="https://akismet.com/get/" method="POST"> 
			<input type="hidden" name="return" value="1"/> 
			<input type="hidden" name="jetpack" value="***REMOVED*** echo (string) class_exists( 'Jetpack' );?>"/>
			<input type="hidden" name="user" value="***REMOVED*** echo esc_attr( $current_user->user_login );?>"/>
			<input type="submit" class="button button-primary" value="***REMOVED*** esc_attr_e( 'Create a new Akismet Key' ); ?>"/>
		</form>
		<br/>
		<a href="***REMOVED*** echo $config_link;?>">***REMOVED*** _e('I already have a key'); ?></a>
	</div>
	<div class="have-key ***REMOVED*** echo $show_key_form ? '' : 'hidden';?>">
		***REMOVED*** if ( !empty($_POST['submit'] ) && $saved_ok ) : ?>
		<div id="message" class="updated fade"><p><strong>***REMOVED*** _e('Settings saved.') ?></strong></p></div>
		***REMOVED*** endif; ?>
		***REMOVED*** if ( isset($_GET['message']) && $_GET['message'] == 'success' ) : ?>
		<div id="message" class="updated fade"><p>***REMOVED*** _e('<strong>Sign up success!</strong> Please check your email for your Akismet API Key and enter it below.') ?></p></div>
		***REMOVED*** endif; ?>
		***REMOVED*** foreach( $ms as $m ) : ?>
		<div class="***REMOVED*** echo $messages[$m]['class']; ?>"><p><strong>***REMOVED*** echo $messages[$m]['text']; ?></strong></p></div>
		***REMOVED*** endforeach; ?>		
		<form action="" method="post" id="akismet-conf">
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="key">***REMOVED*** _e('Akismet API Key');?></label></th>
						<td>
							<input id="key" name="key" type="text" size="15" maxlength="12" value="***REMOVED*** echo esc_attr( get_option('wordpress_api_key') ); ?>" class="regular-text code ***REMOVED*** echo $key_status;?>"><div class="under-input key-status ***REMOVED*** echo $key_status;?>">***REMOVED*** echo isset( $key_status_strings[ $key_status ] ) ? $key_status_strings[ $key_status ] : '';?></div>
							<p class="need-key description">***REMOVED*** printf( __('You must enter a valid Akismet API key here. If you need an API key, you can <a href="%s">create one here</a>'), '#' );?></p>
						</td>
					</tr>
					***REMOVED*** if ( $api_key ):?>
					<tr valign="top">
						<th scope="row">***REMOVED*** _e('Settings');?></th>
						<td>
							<fieldset><legend class="screen-reader-text"><span>***REMOVED*** _e('Settings');?></span></legend>
							<label for="akismet_discard_month" title="***REMOVED*** esc_attr_e( 'Auto-detete old spam' ); ?>"><input name="akismet_discard_month" id="akismet_discard_month" value="true" type="checkbox" ***REMOVED*** echo get_option('akismet_discard_month') == 'true' ? 'checked="checked"':''; ?>> <span>***REMOVED*** _e('Auto-delete spam submitted on posts more than a month old.'); ?></span></label><br>
							<label for="akismet_show_user_comments_approved" title="***REMOVED*** esc_attr_e( 'Show approved comments' ); ?>"><input name="akismet_show_user_comments_approved" id="akismet_show_user_comments_approved" value="true" type="checkbox" ***REMOVED*** echo get_option('akismet_show_user_comments_approved') == 'true' ? 'checked="checked"':''; ?>> <span>***REMOVED*** _e('Show the number of comments you\'ve approved beside each comment author.'); ?></span></label>
							</fieldset>
						</td>
					</tr>
					***REMOVED*** endif; ?>
			...
			***REMOVED*** akismet_nonce_field($akismet_nonce) ?>
			<p class="submit">
				<input type="submit" name="submit" id="submit" class="button button-primary" value="***REMOVED*** _e('Save Changes');?>">
			</p>
		</form>	
		
		***REMOVED*** if ( $api_key ) : ?>
		<h3>***REMOVED*** _e('Server Connectivity'); ?></h3>
		<form action="" method="post" id="akismet-connectivity">
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="key">***REMOVED*** _e('Server Status');?></label></th>
						<td>
						***REMOVED*** if ( !function_exists('fsockopen') || !function_exists('gethostbynamel') ) : ?>
							<p class="key-status failed">***REMOVED*** _e('Network functions are disabled.'); ?></p>
							<p class="description">***REMOVED*** echo sprintf( __('Your web host or server administrator has disabled PHP\'s <code>fsockopen</code> or <code>gethostbynamel</code> functions.  <strong>Akismet cannot work correctly until this is fixed.</strong>  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet\'s system requirements</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?></p>
							***REMOVED*** else :
							...
							<p class="key-status some">***REMOVED*** _e('Unable to reach some Akismet servers.'); ?></p>
							<p class="description">***REMOVED*** echo sprintf( __('A network problem or firewall is blocking some connections from your web server to Akismet.com.  Akismet is working but this may cause problems during times of network congestion.  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet and firewalls</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?></p>
									***REMOVED*** } elseif ( $fail_count > 0 ) { // all connections fail ?>
							<p class="key-status failed">***REMOVED*** _e('Unable to reach any Akismet servers.'); ?></p>
							<p class="description">***REMOVED*** echo sprintf( __('A network problem or firewall is blocking all connections from your web server to Akismet.com.  <strong>Akismet cannot work correctly until this is fixed.</strong>  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet and firewalls</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?></p>
									***REMOVED*** } else { // all connections work ?>
							<p class="key-status valid">***REMOVED***  _e('All Akismet servers are available.'); ?></p>
							<p class="description">***REMOVED*** _e('Akismet is working correctly.  All servers are accessible.'); ?></p>
									***REMOVED*** }
									} else { //can't connect to any server ?>
							<p class="key-status failed">***REMOVED*** _e('Unable to find Akismet servers.'); ?></p>
							<p class="description">***REMOVED*** echo sprintf( __('A DNS problem or firewall is preventing all access from your web server to Akismet.com.  <strong>Akismet cannot work correctly until this is fixed.</strong>  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet and firewalls</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?></p>
								***REMOVED*** }
							endif; ?>							
						</td>
					</tr>
					***REMOVED*** if ( !empty( $servers ) ) : ?>
					<tr valign="top">
						<th scope="row">***REMOVED*** _e('Network Status');?></th>
						<td>
							<table class="network-status">
								<thead>
										<th>***REMOVED*** _e('Akismet server'); ?></th><th>***REMOVED*** _e('Network Status'); ?></th>
								</thead>
								<tbody>
								***REMOVED***
										asort($servers);
										...
										<tr>
											<td align="center">***REMOVED*** echo esc_html( $ip ); ?></td>
											<td class="key-status ***REMOVED*** echo $status ? 'valid' : 'failed'; ?>">***REMOVED*** echo $status ? __('Accessible') : __('Re-trying'); ?></td>
										</tr>										
									***REMOVED*** endforeach; ?>
								</tbody>
							</table>
							<br/>
							<input type="submit" name="check" id="submit" class="button" style="margin-left: 13.3em;" value="***REMOVED*** _e('Check Network Status');?>">
						</td>
					</tr>
					***REMOVED*** endif; ?>
					<tr valign="top">
						<th scope="row">***REMOVED*** _e('Last Checked');?></th>
						<td>
							<p><strong>***REMOVED*** echo get_option('akismet_connectivity_time') ? sprintf( __('%s Ago'), ucwords( human_time_diff( get_option('akismet_connectivity_time') ) ) ) : __( 'Not yet' ); ?></strong></p>
							<p class="description">***REMOVED*** printf( __('You can confirm that Akismet.com is up by <a href="%s" target="_blank">clicking here</a>.'), 'http://status.automattic.com/9931/136079/Akismet-API' ); ?></p>
						</td>
				</tbody>
			</table>
			***REMOVED*** akismet_nonce_field($akismet_nonce) ?>
...
<div class="wrap">***REMOVED***	
	if ( !$api_key ) :?>
	<div id="akismet-warning" class="updated fade"><p><strong>***REMOVED*** _e('Akismet is almost ready.');?></strong> ***REMOVED*** printf( __( 'You must <a href="%1$s">enter your Akismet API key</a> for it to work.' ), $config_link );?></p></div>***REMOVED***
	else :?>
	<iframe src="***REMOVED*** echo esc_url( sprintf( '%s://akismet.com/web/1.0/user-stats.php?blog=%s&api_key=%s', is_ssl()?'https':'http', $blog, $api_key ) ); ?>" width="100%" height="2500px" frameborder="0" id="akismet-stats-frame"></iframe>***REMOVED***
	endif;?>
</div>***REMOVED***
}
...
function akismet_admin_warnings() {
	...
	if (
		...
		if ( get_option( 'akismet_alert_code' ) ) {
			...
			?>
				<div class='error'>
					<p><strong>***REMOVED*** _e( 'Akismet Error Code');?>: ***REMOVED*** echo $alert['code']; ?></strong></p>
					<p>***REMOVED*** esc_html_e( $alert['msg'] ); ?></p>
					<p>***REMOVED*** //FIXME: need to revert this to using __() in next version
