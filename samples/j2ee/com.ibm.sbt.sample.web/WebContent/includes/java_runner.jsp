<div class="btn-group" style="margin-bottom: 20px">
    <button id="runButton" class="btn btn-small" disabled="disabled">Run</button>
    <button id="showHtmlButton" class="btn btn-small" disabled="disabled">Show Generated HTML</button>
</div>

<div class="iframeWrapper" style="border-style:solid;border-width:1px;width=500px;height:450px;border-color:#D3D3D3;overflow:scroll;padding:10px;">
    <iframe id="previewFrame" src="" width="99%" height="99%" style="border-style:none;"></iframe>
</div>
<div id="showHtmlPopout" style="display: hidden;"></div>
<a id="previewLink" href="" target="_blank"></a>
    