...
<body>
	...
	<%
	try {
    	final CustomerManagementService customerManagement = new CustomerManagementService("smartcloud");
		...
		if (!customerList.isEmpty()) {
			...
			JsonJavaObject customerObject = rootObject.getAsObject("Customer");
 			...
 			if(customerObject.get("CustomerState").equals("SUSPENDED")){
			...
			StateChangeListener stateChangeListener = new StateChangeListener(){
				...
				public void stateChanged(JsonEntity jsonEntity){
					try{
						customerManagement.suspendCustomer(custId);
						fout.println("Suspended Customer Id " + custId + "<br/>");
					}catch(Exception e){
						try{
			...
			if(!customerManagement.waitCustomerState(customerId,"ACTIVE",5,1000,stateChangeListener)){
				out.println("Timeout waiting for customer to be suspended");
			}
			
		}
	} catch (Throwable e) {
		e.printStackTrace();
		out.println("Error suspending customer caused by: "+e.getMessage());    		
	}
	%>
	</div>
</body>
