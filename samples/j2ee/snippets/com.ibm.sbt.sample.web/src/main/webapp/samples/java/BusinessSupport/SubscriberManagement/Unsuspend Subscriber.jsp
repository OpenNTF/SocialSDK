...
<html>
<head>
<title>Unsuspend Subscriber</title>
			...
			try {
				...
				StateChangeListener stateChangeListener = new StateChangeListener() {
					...
					public void stateChanged(JsonEntity jsonEntity) {
						try {
							subscriberManagement.unsuspendSubscriber(
									subscriberId);
							fout.println("Unsuspended Subscriber Id "
									+ subscriberId + "<br/>");
						} catch (Exception e) {
							try {
				...
				if (!subscriberManagement.waitSubscriberState(subscriberId,
						"SUSPENDED", 5, 1000, stateChangeListener)) {
					out.println("Timeout waiting for subscriber to be unsuspended");
				}

			} catch (Throwable e) {
				e.printStackTrace();
				out.println("Error unsuspending subscriber caused by: "
						+ e.getMessage());
			}
		%>
	</div>
