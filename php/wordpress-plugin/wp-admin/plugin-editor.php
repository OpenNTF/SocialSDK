...
default:
	...
	?>
***REMOVED*** if (isset($_GET['a'])) : ?>
 <div id="message" class="updated"><p>***REMOVED*** _e('File edited successfully.') ?></p></div>
***REMOVED*** elseif (isset($_GET['phperror'])) : ?>
 <div id="message" class="updated"><p>***REMOVED*** _e('This plugin has been deactivated because your changes resulted in a <strong>fatal error</strong>.') ?></p>
	***REMOVED***
		if ( wp_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $file) ) { ?>
	<iframe style="border:0" width="100%" height="70px" src="***REMOVED*** bloginfo('wpurl'); ?>/wp-admin/plugins.php?action=error_scrape&amp;plugin=***REMOVED*** echo esc_attr($file); ?>&amp;_wpnonce=***REMOVED*** echo esc_attr($_GET['_error_nonce']); ?>"></iframe>
	***REMOVED*** } ?>
</div>
***REMOVED*** endif; ?>
<div class="wrap">
...
<div class="alignright">
	<form action="plugin-editor.php" method="post">
		<strong><label for="plugin">***REMOVED*** _e('Select plugin to edit:'); ?> </label></strong>
...
?>
		...
		***REMOVED*** submit_button( __( 'Select' ), 'button', 'Submit', false ); ?>
	</form>
</div>
<br class="clear" />
</div>
...
?>
		<li***REMOVED*** echo $file == $plugin_file ? ' class="highlight"' : ''; ?>><a href="plugin-editor.php?file=***REMOVED*** echo urlencode( $plugin_file ) ?>&amp;plugin=***REMOVED*** echo urlencode( $plugin ) ?>">***REMOVED*** echo $plugin_file ?></a></li>
***REMOVED*** endforeach; ?>
	</ul>
</div>
<form name="template" id="template" action="plugin-editor.php" method="post">
	***REMOVED*** wp_nonce_field('edit-plugin_' . $file) ?>
		<div><textarea cols="70" rows="25" name="newcontent" id="newcontent" aria-describedby="newcontent-description">***REMOVED*** echo $content; ?></textarea>
		<input type="hidden" name="action" value="update" />
		<input type="hidden" name="file" value="***REMOVED*** echo esc_attr($file) ?>" />
		<input type="hidden" name="plugin" value="***REMOVED*** echo esc_attr($plugin) ?>" />
		<input type="hidden" name="scrollto" id="scrollto" value="***REMOVED*** echo $scrollto; ?>" />
		</div>
		***REMOVED*** if ( !empty( $docs_select ) ) : ?>
		<div id="documentation" class="hide-if-no-js"><label for="docs-list">***REMOVED*** _e('Documentation:') ?></label> ***REMOVED*** echo $docs_select ?> <input type="button" class="button" value="***REMOVED*** esc_attr_e( 'Look Up' ) ?> " onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'http://api.wordpress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=***REMOVED*** echo urlencode( get_locale() ) ?>&amp;version=***REMOVED*** echo urlencode( $wp_version ) ?>&amp;redirect=true'); }" /></div>
		***REMOVED*** endif; ?>
***REMOVED*** if ( is_writeable($real_file) ) : ?>
	***REMOVED*** if ( in_array( $file, (array) get_option( 'active_plugins', array() ) ) ) { ?>
		<p>***REMOVED*** _e('<strong>Warning:</strong> Making changes to active plugins is not recommended. If your changes cause a fatal error, the plugin will be automatically deactivated.'); ?></p>
	***REMOVED*** } ?>
	<p class="submit">
	***REMOVED***
...
***REMOVED*** else : ?>
	<p><em>***REMOVED*** _e('You need to make this file writable before you can save your changes. See <a href="http://codex.wordpress.org/Changing_File_Permissions">the Codex</a> for more information.'); ?></em></p>
***REMOVED*** endif; ?>
</form>
<br class="clear" />
</div>
