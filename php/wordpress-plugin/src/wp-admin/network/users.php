...
function confirm_delete_users( $users ) {
	...
	screen_icon();
	?>
	<h2>***REMOVED*** esc_html_e( 'Users' ); ?></h2>
	<p>***REMOVED*** _e( 'Transfer or delete posts before deleting users.' ); ?></p>
	...
	***REMOVED***
	...
	foreach ( ( $allusers = (array) $_POST['allusers'] ) as $key => $val ) {
		if ( $val != '' && $val != '0' ) {
			...
			if ( !empty( $blogs ) ) {
				?>
				<br /><fieldset><p><legend>***REMOVED*** printf( __( "What should be done with posts owned by <em>%s</em>?" ), $delete_user->user_login ); ?></legend></p>
				***REMOVED***
				foreach ( (array) $blogs as $key => $details ) {
					...
					if ( is_array( $blog_users ) && !empty( $blog_users ) ) {
						...
						<ul style="list-style:none;">
							<li>***REMOVED*** printf( __( 'Site: %s' ), $user_site ); ?></li>
							<li><label><input type="radio" id="delete_option0" name="delete[***REMOVED*** echo $details->userblog_id . '][' . $delete_user->ID ?>]" value="delete" checked="checked" />
							***REMOVED*** _e( 'Delete all posts.' ); ?></label></li>
							<li><label><input type="radio" id="delete_option1" name="delete[***REMOVED*** echo $details->userblog_id . '][' . $delete_user->ID ?>]" value="reassign" />
							***REMOVED*** echo __( 'Attribute all posts to:' ) . '</label>' . $user_dropdown; ?></li>
						</ul>
						***REMOVED***
					}
				}
...
<div class="wrap">
	***REMOVED*** screen_icon(); ?>
	<h2>***REMOVED*** esc_html_e( 'Users' );
	if ( current_user_can( 'create_users') ) : ?>
		<a href="***REMOVED*** echo network_admin_url('user-new.php'); ?>" class="add-new-h2">***REMOVED*** echo esc_html_x( 'Add New', 'user' ); ?></a>***REMOVED***
	endif;
	...
	***REMOVED*** $wp_list_table->views(); ?>
	...
	<form action="" method="get" class="search-form">
		***REMOVED*** $wp_list_table->search_box( __( 'Search Users' ), 'all-user' ); ?>
	</form>
...
***REMOVED*** require_once( ABSPATH . 'wp-admin/admin-footer.php' ); ?>
