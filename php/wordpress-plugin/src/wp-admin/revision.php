...
<div class="wrap">
	***REMOVED*** screen_icon(); ?>
	<h2 class="long-header">***REMOVED*** echo $h2; ?></h2>
</div>
...
<script id="tmpl-revisions-buttons" type="text/html">
	<div class="revisions-previous">
		<input class="button" type="button" value="***REMOVED*** echo esc_attr_x( 'Previous', 'Button label for a previous revision' ); ?>" />
	</div>

	<div class="revisions-next">
		<input class="button" type="button" value="***REMOVED*** echo esc_attr_x( 'Next', 'Button label for a next revision' ); ?>" />
	</div>
</script>
...
<script id="tmpl-revisions-checkbox" type="text/html">
	<div class="revision-toggle-compare-mode">
		<label>
			...
			#>
			/>
			***REMOVED*** esc_attr_e( 'Compare any two revisions' ); ?>
		</label>
	</div>
</script>
...
<script id="tmpl-revisions-meta" type="text/html">
	<# if ( ! _.isUndefined( data.attributes ) ) { #>
		<div class="diff-title">
			<# if ( 'from' === data.type ) { #>
				<strong>***REMOVED*** _ex( 'From:', 'Followed by post revision info' ); ?></strong>
			<# } else if ( 'to' === data.type ) { #>
				<strong>***REMOVED*** _ex( 'To:', 'Followed by post revision info' ); ?></strong>
			<# } #>
			<div class="author-card<# if ( data.attributes.autosave ) { #> autosave<# } #>">
				{{{ data.attributes.author.avatar }}}
				<div class="author-info">
				<# if ( data.attributes.autosave ) { #>
					<span class="byline">***REMOVED*** printf( __( 'Autosave by %s' ),
						'<span class="author-name">{{ data.attributes.author.name }}</span>' ); ?></span>
				<# } else if ( data.attributes.current ) { #>
					<span class="byline">***REMOVED*** printf( __( 'Current Revision by %s' ),
						'<span class="author-name">{{ data.attributes.author.name }}</span>' ); ?></span>
				<# } else { #>
					<span class="byline">***REMOVED*** printf( __( 'Revision by %s' ),
						'<span class="author-name">{{ data.attributes.author.name }}</span>' ); ?></span>
				<# } #>
			...
			<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
				<input  ***REMOVED*** if ( wp_check_post_lock( $post->ID ) ) { ?>
					disabled="disabled"
				***REMOVED*** } else { ?>
				...
				***REMOVED*** } ?>
				<# if ( data.attributes.autosave ) { #>
					type="button" class="restore-revision button button-primary" value="***REMOVED*** esc_attr_e( 'Restore This Autosave' ); ?>" />
				<# } else { #>
					type="button" class="restore-revision button button-primary" value="***REMOVED*** esc_attr_e( 'Restore This Revision' ); ?>" />
				<# } #>
			<# } #>
		</div>
...
<script id="tmpl-revisions-diff" type="text/html">
	...
	<div class="diff-error">***REMOVED*** _e( 'Sorry, something went wrong. The requested comparison could not be loaded.' ); ?></div>
	<div class="diff">
