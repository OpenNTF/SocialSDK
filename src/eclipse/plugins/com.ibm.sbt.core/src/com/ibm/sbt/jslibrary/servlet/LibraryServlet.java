...
public class LibraryServlet extends BaseToolkitServlet {
	...
	/**
	 * Servlet parameter which allows the location of the javascript libraries
	 * to be overridden, the default value is %local_server%/sbt/js/libs.
	...
	static final String DEFAULT_ENDPOINTS = "twitter,connections,smartcloud,domino,sametime"; //$NON-NLS-1$
	...
	public void init(ServletConfig config) throws ServletException {
		...
		String defaultToolkitUrl = getAppParameter(application,
				PARAM_TOOLKIT_URL, DEFAULT_TOOLKIT_URL);
		String defaultToolkitExtUrl = getAppParameter(application,
				PARAM_TOOLKIT_EXT_URL, DEFAULT_TOOLKIT_EXT_URL);
		String defaultJsLibraryUrl = getAppParameter(application,
				PARAM_JS_LIBRARY_URL, DEFAULT_JS_LIBRARY_URL);
		String defaultJavaScriptPath = getAppParameter(application,
				PARAM_JAVASCRIPT_PATH, DEFAULT_JAVASCRIPT_PATH);
		String defaultServiceUrl = getAppParameter(application,
				PARAM_SERVICE_URL, DEFAULT_SERVICE_URL);
		String defaultLibraryUrl = getAppParameter(application,
				PARAM_LIBRARY_URL, DEFAULT_LIBRARY_URL);
		String defaultIFramePath = getAppParameter(application,
				PARAM_IFRAME_PATH, DEFAULT_IFRAME_PATH);

		// load initialisation parameters
		_defaultParams.setToolkitUrl(getInitParameter(config,
				PARAM_TOOLKIT_URL, defaultToolkitUrl));
		_defaultParams.setToolkitJsUrl(PathUtil.concat(_defaultParams
				.getToolkitUrl(), getInitParameter(config,
				PARAM_JAVASCRIPT_PATH, defaultJavaScriptPath), '/'));
		_defaultParams.setToolkitExtUrl(getInitParameter(config,
				PARAM_TOOLKIT_EXT_URL, defaultToolkitExtUrl));
		_defaultParams.setToolkitExtJsUrl(PathUtil.concat(_defaultParams
				.getToolkitExtUrl(), getInitParameter(config,
				PARAM_JAVASCRIPT_PATH, defaultJavaScriptPath), '/'));
		_defaultParams.setServiceUrl(getInitParameter(config,
				PARAM_SERVICE_URL, defaultServiceUrl));
		_defaultParams.setLibraryUrl(getInitParameter(config,
				PARAM_LIBRARY_URL, defaultLibraryUrl));
		_defaultParams.setJsLibraryUrl(getInitParameter(config,
				PARAM_JS_LIBRARY_URL, defaultJsLibraryUrl));
		_defaultParams.setIframeUrl(PathUtil.concat(_defaultParams
				.getToolkitUrl(), getInitParameter(config, PARAM_IFRAME_PATH,
				defaultIFramePath), '/'));
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (library == null) {
				service400(req, resp,
						"Unable to handle request for {0} version:{1}", request
								.getJsLib(), request.getJsVersion());
			} else {
				// handle the request
			...
			if (logger.isLoggable(Level.WARNING)) {
				logger.log(Level.WARNING,
						"Error servicing library GET request", thrown);
			}
		}
	 ...
	 */
	protected LibraryRequest createLibraryRequest(HttpServletRequest req,
	...
	protected SBTEnvironment getDefaultEnvironment(Context context) {
		...
		synchronized (createEnvironmentLock) {
			...
			if (_defaultParams.getEnvironment() == null) {
				...
				String environmentName = getAppParameter(application,
						PARAM_ENVIRONMENT, DEFAULT_ENVIRONMENT);
				SBTEnvironment environment = (SBTEnvironment) context
						.getBean(environmentName);
				if (environment == null) {
					ServletConfig config = getServletConfig();
					String defaultEndpoints = getAppParameter(application,
							PARAM_ENDPOINTS, DEFAULT_ENDPOINTS);
					String endpoints = getInitParameter(config,
							PARAM_ENDPOINTS, defaultEndpoints);
					String defaultClientProps = getAppParameter(application,
							PARAM_CLIENT_PROPERTIES, DEFAULT_CLIENT_PROPERTIES);
					String clientProps = getInitParameter(config,
							PARAM_CLIENT_PROPERTIES, defaultClientProps);

					environmentName = getInitParameter(config,
							PARAM_ENVIRONMENT, DEFAULT_ENVIRONMENT);
	...
	protected AbstractLibrary createLibrary(LibraryRequest request) {
		if (!request.isInited()) {
			throw new IllegalStateException(
					"Access to LibraryRequest before it has been initialised");
		}
