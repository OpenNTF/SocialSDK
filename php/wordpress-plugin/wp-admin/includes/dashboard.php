...
?>
<div id="dashboard-widgets" class="metabox-holder ***REMOVED*** echo $class; ?>">
	<div id='postbox-container-1' class='postbox-container'>
	***REMOVED*** do_meta_boxes( $screen->id, 'normal', '' ); ?>
	</div>
	<div id='postbox-container-2' class='postbox-container'>
	***REMOVED*** do_meta_boxes( $screen->id, 'side', '' ); ?>
	</div>
	<div id='postbox-container-3' class='postbox-container'>
	***REMOVED*** do_meta_boxes( $screen->id, 'column3', '' ); ?>
	</div>
	<div id='postbox-container-4' class='postbox-container'>
	***REMOVED*** do_meta_boxes( $screen->id, 'column4', '' ); ?>
	</div>
</div>
...
?>
	...
	<p class="youhave">***REMOVED*** echo $sentence; ?></p>
	***REMOVED*** do_action( 'wpmuadminresult', '' ); ?>

	<form action="***REMOVED*** echo network_admin_url('users.php'); ?>" method="get">
		<p>
			<input type="search" name="s" value="" size="30" autocomplete="off" />
			***REMOVED*** submit_button( __( 'Search Users' ), 'button', 'submit', false, array( 'id' => 'submit_users' ) ); ?>
		</p>
	</form>

	<form action="***REMOVED*** echo network_admin_url('sites.php'); ?>" method="get">
		<p>
			<input type="search" name="s" value="" size="30" autocomplete="off" />
			***REMOVED*** submit_button( __( 'Search Sites' ), 'button', 'submit', false, array( 'id' => 'submit_sites' ) ); ?>
		</p>
	</form>
***REMOVED***
...
?>
	...
	<form name="post" action="***REMOVED*** echo esc_url( admin_url( 'post.php' ) ); ?>" method="post" id="quick-press">
		<div class="input-text-wrap" id="title-wrap">
			<label class="screen-reader-text prompt" for="title" id="title-prompt-text">***REMOVED*** _e( 'Enter title here' ); ?></label>
			<input type="text" name="post_title" id="title" autocomplete="off" value="***REMOVED*** echo esc_attr( $post->post_title ); ?>" />
		</div>

		***REMOVED*** if ( current_user_can( 'upload_files' ) ) : ?>
		<div id="wp-content-wrap" class="wp-editor-wrap hide-if-no-js wp-media-buttons">
			***REMOVED*** do_action( 'media_buttons', 'content' ); ?>
		</div>
		***REMOVED*** endif; ?>

		<div class="textarea-wrap">
			<label class="screen-reader-text" for="content">***REMOVED*** _e( 'Content' ); ?></label>
			<textarea name="content" id="content" class="mceEditor" rows="3" cols="15">***REMOVED*** echo esc_textarea( $post->post_content ); ?></textarea>
		</div>
		...
		***REMOVED*** if ( $_POST ) : ?>
		...
		wp.media.view.settings.post = ***REMOVED*** echo json_encode( $media_settings ); // big juicy hack. ?>;
		wp.media.editor.add('content');
		***REMOVED*** endif; ?>
		</script>

		<div class="input-text-wrap" id="tags-input-wrap">
			<label class="screen-reader-text prompt" for="tags-input" id="tags-input-prompt-text">***REMOVED*** _e( 'Tags (separate with commas)' ); ?></label>
			<input type="text" name="tags_input" id="tags-input" value="***REMOVED*** echo get_tags_to_edit( $post->ID ); ?>" />
		</div>

		<p class="submit">
			<span id="publishing-action">
				<input type="submit" name="publish" id="publish" accesskey="p" class="button-primary" value="***REMOVED*** current_user_can('publish_posts') ? esc_attr_e('Publish') : esc_attr_e('Submit for Review'); ?>" />
				<span class="spinner"></span>
			</span>
			<input type="hidden" name="action" id="quickpost-action" value="post-quickpress-save" />
			<input type="hidden" name="post_ID" value="***REMOVED*** echo $post_ID; ?>" />
			<input type="hidden" name="post_type" value="post" />
			***REMOVED*** wp_nonce_field('add-post'); ?>
			***REMOVED*** submit_button( __( 'Save Draft' ), 'button', 'save', false, array( 'id' => 'save-post' ) ); ?>
			<input type="reset" value="***REMOVED*** esc_attr_e( 'Reset' ); ?>" class="button" />
			<br class="clear" />
		</p>
...
?>
	<ul>
		<li>***REMOVED*** echo join( "</li>\n<li>", $list ); ?></li>
	</ul>
	<p class="textright"><a href="edit.php?post_status=draft" >***REMOVED*** _e('View all'); ?></a></p>
***REMOVED***
	} else {
...
?>
		...
		<div id="comment-***REMOVED*** echo $comment->comment_ID; ?>" ***REMOVED*** comment_class( array( 'comment-item', wp_get_comment_status($comment->comment_ID) ) ); ?>>
			***REMOVED*** if ( !$comment->comment_type || 'comment' == $comment->comment_type ) : ?>

			***REMOVED*** echo get_avatar( $comment, 50, 'mystery' ); ?>

			<div class="dashboard-comment-wrap">
			<h4 class="comment-meta">
				***REMOVED*** printf( /* translators: 1: comment author, 2: post link, 3: notification if the comment is pending */__( 'From %1$s on %2$s%3$s' ),
			...
			***REMOVED***
			...
			?>
			<div class="dashboard-comment-wrap">
			***REMOVED*** /* translators: %1$s is type of comment, %2$s is link to the post */ ?>
			<h4 class="comment-meta">***REMOVED*** printf( _x( '%1$s on %2$s', 'dashboard' ), "<strong>$type</strong>", $comment_post_link." ".$comment_link ); ?></h4>
			<p class="comment-author">***REMOVED*** comment_author_link(); ?></p>

			***REMOVED*** endif; // comment_type ?>
			<blockquote><p>***REMOVED*** comment_excerpt(); ?></p></blockquote>
			<p class="row-actions">***REMOVED*** echo $actions_string; ?></p>
			</div>
		</div>
***REMOVED***
}
...
function wp_dashboard_quota() {
	...
	?>
	<p class="sub musub">***REMOVED*** _e( 'Storage Space' ); ?></p>
	<div class="table table_content musubtable">
	<table>
		<tr class="first">
			<td class="first b b-posts">***REMOVED*** printf( __( '<a href="%1$s" title="Manage Uploads" class="musublink">%2$sMB</a>' ), esc_url( admin_url( 'upload.php' ) ), number_format_i18n( $quota ) ); ?></td>
			<td class="t posts">***REMOVED*** _e( 'Space Allowed' ); ?></td>
		</tr>
	</table>
	</div>
	...
	<table>
		<tr class="first">
			<td class="b b-comments">***REMOVED*** printf( __( '<a href="%1$s" title="Manage Uploads" class="musublink">%2$sMB (%3$s%%)</a>' ), esc_url( admin_url( 'upload.php' ) ), number_format_i18n( $used, 2 ), $percentused ); ?></td>
			<td class="last t comments***REMOVED*** echo $used_color;?>">***REMOVED*** _e( 'Space Used' );?></td>
		</tr>
	</table>
	</div>
	<br class="clear" />
...
function wp_welcome_panel() {
	...
	<h3>***REMOVED*** _e( 'Welcome to WordPress!' ); ?></h3>
	<p class="about-description">***REMOVED*** _e( 'We&#8217;ve assembled some links to get you started:' ); ?></p>
	<div class="welcome-panel-column-container">
	<div class="welcome-panel-column">
		<h4>***REMOVED*** _e( 'Get Started' ); ?></h4>
		<a class="button button-primary button-hero load-customize hide-if-no-customize" href="***REMOVED*** echo wp_customize_url(); ?>">***REMOVED*** _e( 'Customize Your Site' ); ?></a>
		<a class="button button-primary button-hero hide-if-customize" href="***REMOVED*** echo admin_url( 'themes.php' ); ?>">***REMOVED*** _e( 'Customize Your Site' ); ?></a>
		***REMOVED*** if ( current_user_can( 'install_themes' ) || ( current_user_can( 'switch_themes' ) && count( wp_get_themes( array( 'allowed' => true ) ) ) > 1 ) ) : ?>
			<p class="hide-if-no-customize">***REMOVED*** printf( __( 'or, <a href="%s">change your theme completely</a>' ), admin_url( 'themes.php' ) ); ?></p>
		***REMOVED*** endif; ?>
	</div>
	<div class="welcome-panel-column">
		<h4>***REMOVED*** _e( 'Next Steps' ); ?></h4>
		<ul>
		***REMOVED*** if ( 'page' == get_option( 'show_on_front' ) && ! get_option( 'page_for_posts' ) ) : ?>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-edit-page">' . __( 'Edit your front page' ) . '</a>', get_edit_post_link( get_option( 'page_on_front' ) ) ); ?></li>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-add-page">' . __( 'Add additional pages' ) . '</a>', admin_url( 'post-new.php?post_type=page' ) ); ?></li>
		***REMOVED*** elseif ( 'page' == get_option( 'show_on_front' ) ) : ?>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-edit-page">' . __( 'Edit your front page' ) . '</a>', get_edit_post_link( get_option( 'page_on_front' ) ) ); ?></li>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-add-page">' . __( 'Add additional pages' ) . '</a>', admin_url( 'post-new.php?post_type=page' ) ); ?></li>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-write-blog">' . __( 'Add a blog post' ) . '</a>', admin_url( 'post-new.php' ) ); ?></li>
		***REMOVED*** else : ?>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-write-blog">' . __( 'Write your first blog post' ) . '</a>', admin_url( 'post-new.php' ) ); ?></li>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-add-page">' . __( 'Add an About page' ) . '</a>', admin_url( 'post-new.php?post_type=page' ) ); ?></li>
		***REMOVED*** endif; ?>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-view-site">' . __( 'View your site' ) . '</a>', home_url( '/' ) ); ?></li>
		</ul>
	</div>
	<div class="welcome-panel-column welcome-panel-last">
		<h4>***REMOVED*** _e( 'More Actions' ); ?></h4>
		<ul>
			<li>***REMOVED*** printf( '<div class="welcome-icon welcome-widgets-menus">' . __( 'Manage <a href="%1$s">widgets</a> or <a href="%2$s">menus</a>' ) . '</div>', admin_url( 'widgets.php' ), admin_url( 'nav-menus.php' ) ); ?></li>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-comments">' . __( 'Turn comments on or off' ) . '</a>', admin_url( 'options-discussion.php' ) ); ?></li>
			<li>***REMOVED*** printf( '<a href="%s" class="welcome-icon welcome-learn-more">' . __( 'Learn more about getting started' ) . '</a>', __( 'http://codex.wordpress.org/First_Steps_With_WordPress' ) ); ?></li>
		</ul>
	</div>
	</div>
	</div>
