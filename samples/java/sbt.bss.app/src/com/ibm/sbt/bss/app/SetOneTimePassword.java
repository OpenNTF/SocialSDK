...
public class SetOneTimePassword {
	...
	public SetOneTimePassword(String url, String user, String password) {
		this.url = url;
		this.user = user;
	...
	public static void main(String[] args) {
		if (args.length < 5) {
			System.out.println("Usage: java com.ibm.sbt.bss.app.SetOneTimePassword <url> <user> <password> <loginName> <one_time_password>");
			return;
		}
		...
		SetOneTimePassword cp = null;
		try {
			cp = new SetOneTimePassword(url, user, password);
			...
			System.out.println("Successfully set one time password for: "+loginName);
		} catch (BssException be) {
			System.err.println("Error set one time password for: "+loginName);
